...
import { buildSeoCommandPrompt } from "@/lib/ai/prompts/commands";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { prompt, existingFiles, projectName, conversationHistory, brandNucleus } = await request.json();
    ...
    let systemPrompt = buildSeoCommandPrompt(projectName || "Untitled", filesString, prompt);
    if (brandNucleus) {
    ...
    const readableStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          const msg = error instanceof Error ? error.message : "SEO failed";
